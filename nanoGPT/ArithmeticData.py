...
def create_dataset(op,max_num=128,revert_bit=True):
    ...
    src_data = torch.full((len(tset_input_str), max_in_seq_length), 3,dtype = torch.int32)   # token '0'
    tgt_data = torch.full((len(tset_output_str), max_out_seq_length), 3,dtype = torch.int32)   # token '0'
